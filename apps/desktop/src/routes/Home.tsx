...
import type { AppLocale, AppSettings, GitLabProject, GitLabPublishSettings, GoogleDriveFolder, GooglePublishSettings, GoogleSheetTab, GoogleSpreadsheet, MentionIdentity, Session, SlackPublishSettings, ToolCheck } from '@shared/types'
          ...
          <div className="space-y-3">
            <details className="border border-zinc-800 bg-zinc-900/40 p-3" open>
              <summary className="flex cursor-pointer list-none items-center justify-between text-xs font-medium text-zinc-300">
                <span>Slack</span>
                <span className="text-[11px] font-normal text-zinc-500">{slack.oauthTeamName || 'OAuth'}</span>
              </summary>
            ...
            <details className="border border-zinc-800 bg-zinc-900/40 p-3" open>
              ...
              <div className="mt-3">
                ...
                <div className="mt-2 flex items-center justify-end gap-2">
                  ...
                  <button type="button" onClick={connectGoogleOAuth} disabled={connectingGoogleOAuth} className="rounded bg-zinc-800 px-2.5 py-1.5 text-xs text-zinc-200 hover:bg-zinc-700 disabled:opacity-50">
                    {connectingGoogleOAuth ? 'Connecting...' : 'Connect Google'}
                  </button>
                  {connectingGoogleOAuth && (
                    <button type="button" onClick={cancelGoogleOAuth} className="rounded bg-zinc-900 px-2.5 py-1.5 text-xs text-zinc-300 hover:bg-zinc-800">
                      Cancel OAuth
                    </button>
                  )}
                </div>
                ...
                <div className="mt-3 grid grid-cols-[1fr_auto] items-end gap-2">
                  ...
                  <div className="flex gap-1">
                    <button type="button" onClick={openGoogleDriveFolder} disabled={!parseGoogleDriveFolderInput(google.driveFolderId)} className="rounded bg-zinc-800 px-2.5 py-1.5 text-xs text-zinc-200 hover:bg-zinc-700 disabled:opacity-50">Open</button>
                    <button type="button" onClick={() => loadGoogleFolders()} disabled={refreshingGoogleFolders || !google.token.trim()} className="rounded bg-zinc-800 px-2.5 py-1.5 text-xs text-zinc-200 hover:bg-zinc-700 disabled:opacity-50">
                ...
                <div className="mt-2 grid grid-cols-[1fr_auto] items-end gap-2">
                  <label className="text-xs text-zinc-500">
                    New folder
                    <input value={newGoogleFolderName} onChange={(e) => setNewGoogleFolderName(e.target.value)} placeholder="Loupe QA Evidence" className="mt-1 w-full rounded bg-zinc-950 px-2 py-1.5 text-xs text-zinc-300 outline-none focus:ring-1 focus:ring-blue-600" />
                  </label>
                  <button type="button" onClick={createGoogleFolder} disabled={creatingGoogleFolder || !google.token.trim() || !newGoogleFolderName.trim()} className="rounded bg-zinc-800 px-2.5 py-1.5 text-xs text-zinc-200 hover:bg-zinc-700 disabled:opacity-50">
                    {creatingGoogleFolder ? 'Creating...' : 'Create folder'}
                  </button>
                </div>

                <label className="mt-3 flex items-center gap-2 text-xs text-zinc-400">
                  <input type="checkbox" checked={Boolean(google.updateSheet)} onChange={(e) => { setGoogle({ ...google, updateSheet: e.target.checked }); setGoogleSaved(false) }} className="h-4 w-4 accent-blue-600" />
                ...
                {google.updateSheet && (
                  <div className="mt-2 rounded border border-zinc-800 bg-zinc-950/50 p-2">
                    <div className="grid grid-cols-[1fr_auto] items-end gap-2">
                      ...
                      <div className="flex gap-1">
                        <button type="button" onClick={openGoogleSpreadsheet} disabled={!parseGoogleSpreadsheetInput(google.spreadsheetId)} className="rounded bg-zinc-800 px-2.5 py-1.5 text-xs text-zinc-200 hover:bg-zinc-700 disabled:opacity-50">Open</button>
                        <button type="button" onClick={() => loadGoogleSpreadsheets()} disabled={refreshingGoogleSpreadsheets || !google.token.trim()} className="rounded bg-zinc-800 px-2.5 py-1.5 text-xs text-zinc-200 hover:bg-zinc-700 disabled:opacity-50">
                    ...
                    <div className="mt-2 grid grid-cols-[1fr_auto] items-end gap-2">
                      <label className="text-xs text-zinc-500">
                        Sheet tab
                        <input value={google.sheetName ?? ''} onChange={(e) => { setGoogle({ ...google, sheetName: e.target.value }); setGoogleSaved(false) }} placeholder="Sheet1" className="mt-1 w-full rounded bg-zinc-950 px-2 py-1.5 text-xs text-zinc-300 outline-none focus:ring-1 focus:ring-blue-600" />
                        {googleSheetTabs.length > 0 && (
                          <select value={googleSheetTabs.some(tab => tab.title === google.sheetName) ? google.sheetName : ''} onChange={(e) => { setGoogle({ ...google, sheetName: e.target.value }); setGoogleSaved(false) }} className="mt-1 w-full rounded bg-zinc-950 px-2 py-1.5 text-xs text-zinc-300 outline-none focus:ring-1 focus:ring-blue-600">
                            <option value="">Choose refreshed tab...</option>
                            {googleSheetTabs.map(tab => <option key={tab.sheetId} value={tab.title}>{tab.title}</option>)}
                          </select>
                        )}
                      </label>
                      <button type="button" onClick={() => loadGoogleSheetTabs()} disabled={refreshingGoogleSheetTabs || !google.token.trim() || !google.spreadsheetId?.trim()} className="rounded bg-zinc-800 px-2.5 py-1.5 text-xs text-zinc-200 hover:bg-zinc-700 disabled:opacity-50">
                ...
                <div className="mt-2 flex items-center justify-end gap-2">
                  ...
                  <button onClick={saveGoogleSettings} disabled={savingGoogle} className="rounded bg-zinc-800 px-2.5 py-1.5 text-xs text-zinc-200 hover:bg-zinc-700 disabled:opacity-50">
            ...
            <details className="border border-zinc-800 bg-zinc-900/40 p-3" open>
              ...
              <div className="mt-3">
                <label className="block text-xs text-zinc-500">
                  GitLab base URL
                  <input value={gitlab.baseUrl} onChange={(e) => { setGitLab({ ...gitlab, baseUrl: e.target.value }); setGitLabProjects([]); setGitLabSaved(false) }} placeholder="https://gitlab.com" className="mt-1 w-full rounded bg-zinc-950 px-2 py-1.5 text-xs text-zinc-300 outline-none focus:ring-1 focus:ring-blue-600" />
                </label>
                <div className="mt-2 grid grid-cols-[1fr_180px] gap-2">
                  <div>
                    {gitlab.authType !== 'oauth' && (
                      <label className="text-xs text-zinc-500">
                        GitLab token
                        <input value={gitlab.token} onChange={(e) => { setGitLab({ ...gitlab, token: e.target.value }); setGitLabProjects([]); setGitLabSaved(false) }} type="password" placeholder="glpat-..." className="mt-1 w-full rounded bg-zinc-950 px-2 py-1.5 text-xs text-zinc-300 outline-none focus:ring-1 focus:ring-blue-600" />
                      </label>
                    )}
                  </div>
                  <label className="text-xs text-zinc-500">
                    GitLab auth
                    <select value={gitlab.authType ?? 'pat'} onChange={(e) => { setGitLab({ ...gitlab, authType: e.target.value as GitLabPublishSettings['authType'] }); setGitLabProjects([]); setGitLabSaved(false) }} className="mt-1 w-full rounded bg-zinc-950 px-2 py-1.5 text-xs text-zinc-300 outline-none focus:ring-1 focus:ring-blue-600">
                ...
                {gitlab.authType === 'oauth' && (
                  <div className="mt-2 rounded border border-zinc-800 bg-zinc-950/50 p-2">
                    <div className="grid grid-cols-2 gap-2">
                      <label className="text-xs text-zinc-500">OAuth client ID<input value={gitlab.oauthClientId ?? ''} onChange={(e) => { setGitLab({ ...gitlab, oauthClientId: e.target.value }); setGitLabSaved(false) }} placeholder="Application ID" className="mt-1 w-full rounded bg-zinc-950 px-2 py-1.5 text-xs text-zinc-300 outline-none focus:ring-1 focus:ring-blue-600" /></label>
                      <label className="text-xs text-zinc-500">OAuth client secret<input value={gitlab.oauthClientSecret ?? ''} onChange={(e) => { setGitLab({ ...gitlab, oauthClientSecret: e.target.value }); setGitLabSaved(false) }} type="password" placeholder="Optional for confidential apps" className="mt-1 w-full rounded bg-zinc-950 px-2 py-1.5 text-xs text-zinc-300 outline-none focus:ring-1 focus:ring-blue-600" /></label>
                      <label className="text-xs text-zinc-500">Redirect URI<input value={gitlab.oauthRedirectUri ?? 'http://127.0.0.1:38987/oauth/gitlab/callback'} onChange={(e) => { setGitLab({ ...gitlab, oauthRedirectUri: e.target.value }); setGitLabSaved(false) }} className="mt-1 w-full rounded bg-zinc-950 px-2 py-1.5 text-xs text-zinc-300 outline-none focus:ring-1 focus:ring-blue-600" /></label>
                    </div>
                    <div className="mt-2 flex items-center justify-end gap-2">
                      {gitlab.token.trim() && <span className="text-xs text-emerald-300">Connected</span>}
                      <button type="button" onClick={connectGitLabOAuth} disabled={savingGitLab || connectingGitLabOAuth || !gitlab.baseUrl.trim() || !gitlab.oauthClientId?.trim()} className="rounded bg-zinc-800 px-2.5 py-1.5 text-xs text-zinc-200 hover:bg-zinc-700 disabled:opacity-50">
                        {connectingGitLabOAuth ? 'Connecting...' : 'Connect OAuth'}
                      </button>
                      {connectingGitLabOAuth && <button type="button" onClick={cancelGitLabOAuth} className="rounded bg-zinc-900 px-2.5 py-1.5 text-xs text-zinc-300 hover:bg-zinc-800">Cancel OAuth</button>}
                    </div>
                  </div>
                )}
                <div className="mt-2 grid grid-cols-[1fr_auto] items-end gap-2">
                  <label className="text-xs text-zinc-500">
                    ...
                    {gitlabProjects.length > 0 ? (
                      <select value={gitlab.projectId} onChange={(e) => { setGitLab({ ...gitlab, projectId: e.target.value }); setGitLabSaved(false) }} className="mt-1 w-full rounded bg-zinc-950 px-2 py-1.5 text-xs text-zinc-300 outline-none focus:ring-1 focus:ring-blue-600">
                      ...
                      <input value={gitlab.projectId} onChange={(e) => { setGitLab({ ...gitlab, projectId: e.target.value }); setGitLabSaved(false) }} placeholder="group/project" className="mt-1 w-full rounded bg-zinc-950 px-2 py-1.5 text-xs text-zinc-300 outline-none focus:ring-1 focus:ring-blue-600" />
                    )}
                  </label>
                  <button type="button" onClick={() => loadGitLabProjects()} disabled={refreshingGitLabProjects || !gitlab.baseUrl.trim() || !gitlab.token.trim()} className="rounded bg-zinc-800 px-2.5 py-1.5 text-xs text-zinc-200 hover:bg-zinc-700 disabled:opacity-50">
                    {refreshingGitLabProjects ? 'Refreshing...' : 'Refresh projects'}
                  </button>
                </div>
                <div className="mt-2 grid grid-cols-2 gap-2">
                  <label className="text-xs text-zinc-500">Labels<input value={gitlabLabelsInput} onChange={(e) => { setGitLabLabelsInput(e.target.value); setGitLabSaved(false) }} placeholder="loupe, qa-evidence" className="mt-1 w-full rounded bg-zinc-950 px-2 py-1.5 text-xs text-zinc-300 outline-none focus:ring-1 focus:ring-blue-600" /></label>
                  <label className="text-xs text-zinc-500">GitLab fallback usernames<input value={gitlabMentionsInput} onChange={(e) => { setGitLabMentionsInput(e.target.value); setGitLabSaved(false) }} placeholder="@qa, @lead" className="mt-1 w-full rounded bg-zinc-950 px-2 py-1.5 text-xs text-zinc-300 outline-none focus:ring-1 focus:ring-blue-600" /></label>
                </div>
                <label className="mt-2 block text-xs text-zinc-500">
                  Default GitLab mode
                  <select value={gitlab.mode} onChange={(e) => { setGitLab({ ...gitlab, mode: e.target.value as GitLabPublishSettings['mode'] }); setGitLabSaved(false) }} className="mt-1 w-full rounded bg-zinc-950 px-2 py-1.5 text-xs text-zinc-300 outline-none focus:ring-1 focus:ring-blue-600">
                ...
                <label className="mt-2 block text-xs text-zinc-500">
                  GitLab email lookup
                  <select value={gitlab.emailLookup ?? 'off'} onChange={(e) => { setGitLab({ ...gitlab, emailLookup: e.target.value as GitLabPublishSettings['emailLookup'] }); setGitLabSaved(false) }} className="mt-1 w-full rounded bg-zinc-950 px-2 py-1.5 text-xs text-zinc-300 outline-none focus:ring-1 focus:ring-blue-600">
                ...
                <label className="mt-2 flex items-center gap-2 text-xs text-zinc-400">
                  <input type="checkbox" checked={Boolean(gitlab.confidential)} onChange={(e) => { setGitLab({ ...gitlab, confidential: e.target.checked }); setGitLabSaved(false) }} className="h-4 w-4 accent-blue-600" />
                ...
                <div className="mt-2 rounded border border-zinc-800 bg-zinc-950/50 p-2">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex gap-1">
                      <button
                        type="button"
                        onClick={() => refreshGitLabUsers(false)}
                        disabled={refreshingGitLabUsers || !gitlab.token.trim() || !gitlab.projectId.trim()}
                        title={!gitlab.token.trim() ? 'Set a GitLab token or connect OAuth first.' : !gitlab.projectId.trim() ? 'Choose a GitLab project first.' : undefined}
                      ...
                      <button
                        type="button"
                        onClick={() => refreshGitLabUsers(true)}
                        disabled={refreshingGitLabUsers || !gitlab.token.trim() || !gitlab.projectId.trim()}
                        title={!gitlab.token.trim() ? 'Set a GitLab token or connect OAuth first.' : !gitlab.projectId.trim() ? 'Choose a GitLab project first.' : 'Force GitLab email lookup through /users/:id.'}
                        className="rounded bg-emerald-800 px-2.5 py-1.5 text-xs text-emerald-50 hover:bg-emerald-700 disabled:opacity-50"
                      >
                        Fetch emails
                      </button>
                    </div>
                ...
                <div className="mt-2 flex items-center justify-end gap-2">
                  ...
                  <button onClick={saveGitLabSettings} disabled={savingGitLab} className="rounded bg-zinc-800 px-2.5 py-1.5 text-xs text-zinc-200 hover:bg-zinc-700 disabled:opacity-50">
            ...
            <details className="border border-zinc-800 bg-zinc-900/40 p-3" open>
              ...
              <div className="mt-3">
                <div className="mb-2 flex items-center justify-between gap-2">
                  ...
                  <button type="button" onClick={() => addMentionIdentity()} className="rounded bg-zinc-800 px-2.5 py-1.5 text-xs text-zinc-200 hover:bg-zinc-700">{t('settings.mentionIdentities.addPerson')}</button>
                </div>
                <div className="overflow-x-auto rounded border border-zinc-800 bg-zinc-950">
                  ...
                  {mentionIdentities.length === 0 ? (
                    ...
                    <div key={`${identity.id}-${index}`} className="grid min-w-[920px] grid-cols-[1.1fr_1.2fr_1fr_1fr_1.2fr_72px] items-start gap-2 border-b border-zinc-900 px-2 py-1.5 last:border-b-0">
                      <div className="min-w-0">
                        <input value={identity.displayName} onChange={(e) => updateMentionIdentity(index, { displayName: e.target.value })} className="w-full min-w-0 rounded bg-zinc-900 px-2 py-1 text-xs text-zinc-200 outline-none focus:ring-1 focus:ring-blue-600" />
                        <div className="mt-1 flex min-h-5 flex-wrap gap-1">
                          <MentionIdentityBadges identity={identity} />
                        </div>
                      </div>
                      <input value={identity.email ?? ''} onChange={(e) => updateMentionIdentity(index, { email: e.target.value.trim().toLowerCase() || undefined })} placeholder="name@example.com" className="min-w-0 rounded bg-zinc-900 px-2 py-1 text-xs text-zinc-200 outline-none focus:ring-1 focus:ring-blue-600" />
                      <input value={identity.slackUserId ?? ''} onChange={(e) => updateMentionIdentity(index, { slackUserId: e.target.value.trim() || undefined })} placeholder="U123..." className="min-w-0 rounded bg-zinc-900 px-2 py-1 text-xs text-zinc-200 outline-none focus:ring-1 focus:ring-blue-600" />
                      <input value={identity.gitlabUsername ? `@${identity.gitlabUsername}` : ''} onChange={(e) => updateMentionIdentity(index, { gitlabUsername: e.target.value.trim().replace(/^@/, '') || undefined })} placeholder="@username" className="min-w-0 rounded bg-zinc-900 px-2 py-1 text-xs text-zinc-200 outline-none focus:ring-1 focus:ring-blue-600" />
                      <input value={identity.googleEmail ?? ''} onChange={(e) => updateMentionIdentity(index, { googleEmail: e.target.value.trim().toLowerCase() || undefined })} placeholder="name@example.com" className="min-w-0 rounded bg-zinc-900 px-2 py-1 text-xs text-zinc-200 outline-none focus:ring-1 focus:ring-blue-600" />
                      <button type="button" onClick={() => removeMentionIdentity(index)} className="rounded bg-zinc-900 px-2 py-1 text-xs text-zinc-400 hover:bg-red-950 hover:text-red-100">{t('common.remove')}</button>
                    </div>
                  ))}
                </div>
                {activeSlackUsers.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {activeSlackUsers.filter(user => !mentionIdentities.some(identity => identity.slackUserId === user.id)).slice(0, 12).map(user => {
                      const label = user.displayName || user.realName || user.name || user.id
                      return <button key={user.id} type="button" onClick={() => addMentionIdentity({ displayName: label, email: user.email, slackUserId: user.id })} className="rounded bg-zinc-900 px-2 py-1 text-[11px] text-zinc-400 hover:bg-zinc-800 hover:text-zinc-200">{t('settings.mentionIdentities.addSlack', { name: label })}</button>
                    })}
                  </div>
                )}
                ...
                <div className="mt-2 flex items-center justify-end gap-2">
                  ...
                  <button type="button" onClick={importMentionIdentities} className="rounded bg-zinc-900 px-2.5 py-1.5 text-xs text-zinc-300 hover:bg-zinc-800">{t('common.import')}</button>
                  <button type="button" onClick={exportMentionIdentities} className="rounded bg-zinc-900 px-2.5 py-1.5 text-xs text-zinc-300 hover:bg-zinc-800">{t('common.export')}</button>
                  <button type="button" onClick={saveMentionIdentities} disabled={savingMentionIdentities} className="rounded bg-zinc-800 px-2.5 py-1.5 text-xs text-zinc-200 hover:bg-zinc-700 disabled:opacity-50">
