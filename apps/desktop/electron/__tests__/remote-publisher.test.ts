...
describe('remote publisher', () => {
  it('skips local exports', async () => {
    const manifest = buildExportManifest({
      session: session(),
      bugs: [bug()],
      files: [{ bugId: 'b1', videoPath: '/exports/b1.mp4', previewPath: '/exports/b1.jpg', screenshotPath: null, screenshotHash: null, logcatPath: null }],
      outDir: '/exports',
  ...
  it('returns a failed publish result instead of throwing when a remote target is not configured', async () => {
    ...
    appSettings.profiles[0].google = { ...appSettings.profiles[0].google, token: '', refreshToken: '', driveFolderId: '' }
