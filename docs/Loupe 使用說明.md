...
如果自動掃描找不到手機，可以用 Loupe 左側下方的兩個手動區塊：

- **Pair Android manually**：輸入 Android 無線偵錯頁面中 **Pair device with pairing code** 顯示的 `IP:pairing-port` 與六位數配對碼。
- **Add Wi-Fi device manually**：配對完成後，再輸入 Android 顯示的 `IP:connect-port` 或 ready/connect 位址並按 **connect**。

請不要把 pairing port 直接拿去 connect，否則會連線失敗。
...
Android 錄製時，每個 marker 也會額外保存當下最近幾筆 logcat，方便 QA 回看 bug 當下的系統訊息。
...
如果 marker 有擷取到 Android logcat，marker 編輯區會顯示 logcat 預覽。這個預覽主要是給 QA 快速確認 bug 當下有沒有 crash、ANR、Activity 切換或其他關鍵訊息。

輸出前會跳出確認視窗，可調整輸出資料夾、tester 和 test note。

如果 marker 有 logcat，確認視窗也可以勾選是否要一起輸出 logcat 文字檔。這個 logcat 會另外輸出成 sidecar `.txt` 檔，不會直接塞進影片畫面。
...
- 一個 MP4 裁切影片
- 一張 3x3 預覽圖
- 可選擇另外輸出一個對應的 logcat 文字檔（`.logcat.txt`）
...
logcat 不會直接寫進影片 caption。若有勾選輸出 logcat，Loupe 會把該 marker 的 logcat 另外存成同名 sidecar 文字檔，方便工程師後續查看完整內容。
