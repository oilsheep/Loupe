...
Loupe 是給 QA 測試使用的錄影工具。它可以錄 Android 手機，也可以錄 PC/Mac 畫面，包含整個螢幕或單一視窗。測試中可以快速打點，停止後再補 note、調整裁切範圍、錄語音註記，最後輸出 bug 影片、六張圖證據圖、HTML/PDF 報告與 `summery.txt`。
...
- **PC/Mac screen 或 window**：錄製一個完整螢幕，或只錄單一應用程式視窗，適合 PC build、網頁、後台工具或非手機流程。
...
3. 點選要錄製的螢幕或視窗卡片。
4. 如果選的是整個螢幕，確認該螢幕出現綠色外框。
5. 輸入 build 或測試版本。
6. 按 **Start session**。
7. 錄製中，整個螢幕來源會顯示紅色外框；單一視窗來源不會顯示外框。
8. 使用熱鍵或彩色標籤按鈕打點。
9. 測試完成後按 **Stop**。

單一視窗錄製只會錄選到的視窗內容。若視窗最小化、被系統隱藏，或應用程式使用受保護內容，錄影結果可能會變黑或停止更新。
...
1. 選擇 PC/Mac 螢幕、PC/Mac 視窗、USB Android 或 Wi-Fi Android。
2. 輸入 build 或測試版本。
3. 可選填測試註記。
4. 按 **Start session**。

Android 會開啟可操作的鏡像視窗；PC/Mac 會錄製選定的完整螢幕或單一視窗。
